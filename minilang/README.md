...
-----------------------
- Explicar la separación de fases: el Lexer transforma el texto en tokens;
  el Parser valida la estructura y construye el AST; el Intérprete recorre el AST
  y ejecuta las instrucciones apoyado en una tabla de símbolos.
- Mostrar ejemplos de tokens generados (ejecutar `lexer.py` como script).
- Mostrar cómo se detecta un error léxico (identificador inválido) y un error sintáctico.
